...
Respond in the user's language. Structure the response exactly per the required sections below. Use Mermaid diagrams in fenced `mermaid` blocks. Use fenced code blocks with language tags for all concrete representations (math, code, notation). Bold key terms for scannability. No preamble, no postamble.
